...
// EmailJS configuration via Vite environment variables (required).
const EMAILJS_SERVICE_ID = import.meta.env.VITE_EMAILJS_SERVICE_ID;
const EMAILJS_TEMPLATE_ID = import.meta.env.VITE_EMAILJS_TEMPLATE_ID;
const EMAILJS_PUBLIC_KEY = import.meta.env.VITE_EMAILJS_PUBLIC_KEY;
// Recipient used by templates (optional - template must accept `to_email` if using this field)
const CONTACT_RECIPIENT_EMAIL = import.meta.env.VITE_CONTACT_RECIPIENT_EMAIL;

// Validate required env variables at runtime and surface console errors if missing
let EMAILJS_CONFIG_VALID = true;
(() => {
  const missing = [];
  ...
  if (!CONTACT_RECIPIENT_EMAIL) missing.push('VITE_CONTACT_RECIPIENT_EMAIL');

  if (missing.length > 0) {
    console.error(`[EmailJS] Missing environment variables: ${missing.join(', ')}.\nPlease add them to your .env (see .env.example) and restart the dev server.`);
    EMAILJS_CONFIG_VALID = false;
  }
})(); 
...
const ContactForm = () => {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    // Abort if EmailJS env configuration is missing
    if (!EMAILJS_CONFIG_VALID) {
      console.error('[EmailJS] Attempted to send email but configuration is missing.');
      setIsSubmitting(false);
      setErrorMessage('Unable to send message: Email service not configured. Please try again later or contact surenderdubey9582@gmail.com.');
      setSuccess("");
      return;
    }
    ...
    emailjs
      ...
      .then(
        ...
        (error) => {
          ...
          setIsSubmitting(false);
          setErrorMessage('Failed to send message. Please try again later or contact surenderdubey9582@gmail.com.');
        }
      );
  };
  ...
  return (
    <div className="relative">
      ...
      {errorMessage && (
        ...
        >
          <FaTimes className="text-red text-lg" />
      ...
      <form ref={form} onSubmit={sendEmail} className="flex flex-col gap-6">
        {/* Hidden recipient (used by EmailJS template if configured) */}
